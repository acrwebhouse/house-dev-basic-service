...
function editHouseDev(id,name,companyId,status,serviceCharge,city,area,owner,address,saleType,source,hostName,hostPhone,hostGender,remark, callback) {
    if (id.length == 24){
        const updateData = {
            name,
...
function getHouseDevListQueryInfos(isDelete,name,status,companyId,owner,city,area,saleType){
    const queryInfo = {
        isDelete
    }
    ...
    if(utilsValue.isValid(status) && utilsValue.isNumber(status)){

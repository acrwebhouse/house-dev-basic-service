exports.on = function(app) {
    ...
    app.put(preRestApi + '/editHouseDev', function(req, res) {
       /*#swagger.parameters['obj'] = {
        ...
        houseDev.editHouseDev(id,name,companyId,state,serviceCharge,city,area,owner,address,saleType,source,hostName,hostPhone,hostGender,remark,(result,data)=> {
            response.status = result;
            response.data = data
            res.send(response);
        })
    ...
    app.get(preRestApi + '/getHouseDevList', function(req, res) {
        ...
        */ 
        ...
        const queryInfos = houseDev.getHouseDevListQueryInfos(isDelete,name,state,companyId,owner,city,area,saleType)
        let sort;
        try{
